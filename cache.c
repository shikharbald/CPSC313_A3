...
static int cache_line_check_validity_and_tag(cache_line_t *cache_line, intptr_t tag) {
   ...
   return (cache_line->is_valid == 1 && cache_line->tag == tag);
...
static long cache_line_retrieve_data(cache_line_t *cache_line, size_t offset) {
  ...
  long data_integer = *((long*) (cache_line->data+offset));
    return data_integer; 
...
static cache_line_t *find_available_cache_line(cache_t *cache, cache_set_t *cache_set) {
  unsigned int max_i = cache->associativity-1;
  cache_line_t * result;
  if(cache->policies == CACHE_REPLACEMENTPOLICY_LRU){ 
  for(int i = 0; i < cache->associativity; i++){
     if(!cache_set->cache_lines[i]->is_valid){
      result =  cache_line_make_mru(cache_set, i);
       
     }
   }
result =  cache_line_make_mru(cache_set, max_i);
}

else{
int rnd_index = (rand() % (max_i + 1));
result = cache_line_make_mru(cache_set, rnd_index);
...
long cache_read(cache_t *cache, long *address) {
  ...
  uintptr_t address_new =(uintptr_t) address;
  
  // The s_index or s bits
  int s_index_unmasked = address_new >> cache->cache_index_shift;
  int s_index = s_index_unmasked & cache->cache_index_mask;
  
  // The tag bits or t bits
  uintptr_t address_tag = address_new >> cache->tag_shift; 
 
  // The blocks or b bits
  uintptr_t  address_block = address_new & cache->block_offset_mask;
                                                                 

  cache_set_t * set = &cache->sets[s_index];
  cache_line_t * cachline = cache_set_find_matching_line(cache, set, address_tag);
  
  if (!cachline){
    cache->miss_count++;
    cachline = cache_set_add(cache, set,  address_new, address_tag);                                                       

} 

  cache->access_count++;
  long data = cache_line_retrieve_data(cachline, address_block);
